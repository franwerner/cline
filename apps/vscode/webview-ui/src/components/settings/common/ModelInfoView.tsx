		...
		.map((tier, index, arr) => {
			...
			return (
				<span key={`tier-${tier.contextWindow}`} style={{ paddingLeft: "15px" }}>
					{formatPrice(price)}/millón de tokens (
		...
		<div style={{ marginTop: 4 }}>
			...
			<CollapsibleContent $isExpanded={advancedExpanded}>
				<AdvancedSection>
					{/* Capabilities */}
					<AdvancedRow>
						<AdvancedLabel>Imágenes</AdvancedLabel>
						<AdvancedValue>{hasImages ? "Sí" : "No"}</AdvancedValue>
					</AdvancedRow>
					<AdvancedRow>
						<AdvancedLabel>Navegador</AdvancedLabel>
						<AdvancedValue>{hasBrowser ? "Sí" : "No"}</AdvancedValue>
					</AdvancedRow>
					{!isGemini && (
						<AdvancedRow>
							<AdvancedLabel>Caché de prompts</AdvancedLabel>
							<AdvancedValue>{hasCaching ? "Sí" : "No"}</AdvancedValue>
						</AdvancedRow>
					)}
					...
					{hasTiers && (
						<div style={{ marginTop: 8 }}>
							<div style={{ fontWeight: 500, marginBottom: 4 }}>Precios por niveles:</div>
							{modelInfo.tiers && (
								<>
									<div>
					...
					{showProviderRouting && onProviderSortingChange && (
						<ProviderRoutingContainer>
							<ProviderRoutingLabel>Enrutamiento de proveedores</ProviderRoutingLabel>
							<VSCodeDropdown
								onChange={(e: any) => onProviderSortingChange(e.target.value)}
								style={{ width: "100%" }}
								value={providerSorting || ""}>
								<VSCodeOption value="">Predeterminado</VSCodeOption>
								<VSCodeOption value="price">Precio</VSCodeOption>
								<VSCodeOption value="throughput">Rendimiento</VSCodeOption>
								<VSCodeOption value="latency">Latencia</VSCodeOption>
							</VSCodeDropdown>
							<p
								style={{
									fontSize: "11px",
								...
								{!providerSorting &&
									"Equilibra la carga entre proveedores (AWS, Google Vertex, etc.), priorizando el precio y teniendo en cuenta la disponibilidad"}
								{providerSorting === "price" && "Ordenar por precio, priorizando el proveedor de menor coste"}
								{providerSorting === "throughput" &&
									"Ordenar por rendimiento, priorizando el mayor rendimiento (puede aumentar el coste)"}
								{providerSorting === "latency" && "Ordenar por tiempo de respuesta, priorizando la menor latencia"}
							</p>
						</ProviderRoutingContainer>
					)}
				</AdvancedSection>
